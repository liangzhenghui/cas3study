...
{
	static final Logger Log = Logger.getLogger(CasClient.class.getName());
	...
	public String login (String serviceUrl, String username, String password) throws CasAuthenticationException, CasProtocolException
	{
		...
		if (lt == null)
		{
			Log.info(TAG+"Cannot retrieve LT from CAS. Aborting authentication for '" + username + "'");
		...
		else
		{
			...
			Log.info(TAG+"POST " + casBaseURL + CAS_LOGIN_URL_PART + "?service=" + serviceUrl);
			...
			try
			{
				...
				Log.info(TAG+"POST RESPONSE STATUS=" + response.getStatusLine().getStatusCode() + " : " + response.getStatusLine().toString());
				...
				{
					Log.info(TAG+"Authentication to service '" + serviceUrl + "' unsuccessul for username '" + username + "'.");
					throw new CasAuthenticationException ("Authentication to service '" + serviceUrl + "' unsuccessul for username '" + username + "'.");
				}
				else
					Log.info(TAG+"Authentication to service '" + serviceUrl + "' successul for username '" + username + "'.");
			} 
			catch (IOException e) 
			{
				Log.info(TAG+"IOException trying to login : " + e.getMessage());
	...
	public boolean logout ()
	{
		...
		try
		{
			...
			Log.info(TAG+response.getStatusLine().toString());
		} 
		catch (Exception e)
		{
			Log.info(TAG+ "Exception trying to logout : " + e.getMessage());
			logoutSuccess = false;
		} 
	...
	public String validate (String serviceUrl, String serviceTicket) throws CasAuthenticationException, CasProtocolException
	{
		...
		Log.info(TAG+"VALIDATE : " + httpPost.getRequestLine());
		...
		try
		{
			...
			httpPost.setEntity (new UrlEncodedFormEntity(nvps));
			HttpResponse response = httpClient.execute (httpPost);
			Log.info(TAG+"VALIDATE RESPONSE : " + response.getStatusLine().toString());
			...
			{   
				...
				Log.info(TAG+"VALIDATE OK YOU ARE : " + username);
				entity.consumeContent();
			}
		} 
		catch (Exception e)
		{
			Log.info(TAG+"Could not validate: " + e.getMessage ());
	...
	protected String getLTFromLoginForm (String serviceUrl)
	{
		...
		try
		{
			Log.info(TAG+"Ready to get LT from " + casBaseURL + CAS_LOGIN_URL_PART + "?service=" + serviceUrl);
			HttpResponse response = httpClient.execute (httpGet);
			Log.info(TAG+"Response = " + response.getStatusLine().toString());
			if (response.getStatusLine().getStatusCode() != HttpStatus.SC_OK)
			{
				Log.info(TAG+"Could not obtain LT token from CAS: " + response.getStatusLine().getStatusCode() + " / " + response.getStatusLine());
			} 
			else
			{
				HttpEntity entity = response.getEntity();
		...
		{
			//Log.info(TAG+ "Getting LT client protocol exception"e);
		} 
		catch (IOException e) 
		{
			//Log.info(TAG+ "Getting LT io exception",e);
		}
	 ...
	 */
	public String extractUser (InputStream dataStream)
	{
		...
		{
			Log.info(TAG+e.getLocalizedMessage());
		}
		return user;
	}
